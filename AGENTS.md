...
tool installers, and an optional XXH remote-shell payload.

## Working rules

- Preserve user-owned and ignored state such as `.env`, `.zsh_history`,
  `.cache/`, `.antidote/`, and `modules/xxh-plugin/build/`.
...
- Antidote, FNM, pnpm-shell-completion, and XXH versions are intentionally
  pinned in setup scripts.
- `modules/xxh-plugin/manifest.json` is generated by the XXH build from payload
  content. A changed payload should change this manifest; identical builds
  must not.
- The XXH build must not contain nested `.git` directories, tests, upstream
  documentation, or other development-only content.
...
For XXH runtime changes, also run:
...
The second build must produce the same manifest. Confirm payload size and file
count remain reasonable.
...
variables, installation behavior, or common commands change. Update
`docs/xxh-roadmap.md` when an XXH roadmap item is completed or newly identified.
