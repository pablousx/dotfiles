...
configure_modules_interactively() {
    ...
    DISABLE_PROMPT_VALUE="$(prompt_module "  Enable the custom prompt?" "$DISABLE_PROMPT_VALUE")"
    DISABLE_PLUGINS_VALUE="$(prompt_module "  Enable shell plugins?" "$DISABLE_PLUGINS_VALUE")"
...
usage() {
    cat <<'EOF'
Usage: ./setup.sh [--profile auto|zsh|omarchy] [--all | COMPONENT...]

The default profile is detected from the host distribution, including automation.
Supported: macOS, Debian/Ubuntu, Fedora/RHEL, Arch/Omarchy, openSUSE/SUSE families.
Unrecognized distributions fail before setup makes changes.
For Bash options: ./setup.sh --profile omarchy --help
