...
export DOTFILES_DIR="${DOTFILES_DIR:-${${(%):-%x}:A:h:h:h}}"
...
if [[ "${DISABLE_PLUGINS:-false}" != true ]]; then
  if [[ -r "$DOTFILES_DIR/profiles/zsh/modules/plugins.zsh" ]]; then
    source "$DOTFILES_DIR/profiles/zsh/modules/plugins.zsh"
  else
    print -u2 "Plugin bundle missing. Run: bash $DOTFILES_DIR/setup/zsh.sh install $DOTFILES_DIR"
  fi
  [[ -r "$DOTFILES_DIR/profiles/zsh/modules/platform.zsh" ]] && source "$DOTFILES_DIR/profiles/zsh/modules/platform.zsh"
fi
...
if [[ "${DISABLE_ALIASES:-false}" != true ]]; then
  source "$DOTFILES_DIR/profiles/zsh/modules/aliases.zsh"
fi
if [[ "${DISABLE_PRINT_ALIAS_COMPLETION:-false}" != true ]]; then
  source "$DOTFILES_DIR/profiles/zsh/modules/print-alias-completion.zsh"
fi
if [[ "${DISABLE_EXPAND_ALIAS:-false}" != true ]]; then
  source "$DOTFILES_DIR/profiles/zsh/modules/expand-alias.zsh"
fi
if [[ "${DISABLE_PROMPT:-false}" != true ]]; then
  source "$DOTFILES_DIR/profiles/zsh/modules/prompt.zsh"
fi
